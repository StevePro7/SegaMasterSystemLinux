...
namespace ExtractCheckpoints
{
	...
	{
		private const string root = @"E:/GitHub/StevePro8/SegaMasterSystemLinux/TurtleBridge/level/levels/lego/10-points_output/";
		...
		public void Process(string dirX)
		{
			string path = String.Format("{0}{1}/", root, dirX);
			var dirs = Directory.GetDirectories(path);
			var dirn = dirs[0];
			var dirt = dirn.Replace(path, String.Empty);
			var name = dirt.Substring(0, 10);
			Build(dirX, dirn, dirt, name);
		}

		public void Build(string dirX, string dirn, string dirt, string name)
		{
			...
			string path = String.Format("{0}{1}/{2}", root, dirn, name);
