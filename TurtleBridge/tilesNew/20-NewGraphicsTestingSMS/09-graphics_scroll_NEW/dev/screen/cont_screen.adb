M:cont_screen
F:G$screen_cont_screen_load$0_0$0({2}DF,SV:S),Z,0,0,0,0,0
F:G$screen_cont_screen_update$0_0$0({2}DF,SV:S),Z,0,0,0,0,0
S:Lcont_screen.screen_cont_screen_update$screen_type$1_0$25({2}DG,SC:U),B,1,4
S:Lcont_screen.screen_cont_screen_update$so$1_0$26({2}DG,STtag_struct_scroll_object:S),R,0,0,[]
S:Lcont_screen.screen_cont_screen_update$po$1_0$26({2}DG,STtag_struct_player_object:S),R,0,0,[]
S:Lcont_screen.screen_cont_screen_update$lo$1_0$26({2}DG,STtag_struct_level_object:S),R,0,0,[]
S:Lcont_screen.screen_cont_screen_update$fo$1_0$26({2}DG,STtag_struct_frame_object:S),R,0,0,[]
S:Lcont_screen.screen_cont_screen_update$go$1_0$26({2}DG,STtag_struct_game_object:S),R,0,0,[]
S:Lcont_screen.screen_cont_screen_update$input$1_0$26({1}SC:U),R,0,0,[l]
S:Lcont_screen.screen_cont_screen_update$deltaX$1_0$26({1}SC:U),B,1,-4
S:Lcont_screen.screen_cont_screen_update$value$1_0$26({1}SC:U),B,1,-1
S:Lcont_screen.screen_cont_screen_update$collision$1_0$26({1}SC:S),R,0,0,[l]
S:Lcont_screen.screen_cont_screen_update$deltaY$1_0$26({1}SC:S),R,0,0,[b]
S:Lcont_screen.screen_cont_screen_update$scroll_state$1_0$26({1}SC:U),R,0,0,[l]
S:Lcont_screen.screen_cont_screen_update$sloc0$0_1$0({1}SC:U),B,1,-4
S:Lcont_screen.screen_cont_screen_update$sloc1$0_1$0({1}SC:U),B,1,-1
S:Lcont_screen.screen_cont_screen_update$sloc2$0_1$0({2}SI:U),B,1,-3
T:Fcont_screen$tag_struct_player_object[({0}S:S$initX$0_0$0({1}SC:U),Z,0,0)({1}S:S$posnX$0_0$0({2}SI:U),Z,0,0)({3}S:S$tileX$0_0$0({2}SI:U),Z,0,0)({5}S:S$lookX$0_0$0({1}SC:U),Z,0,0)({6}S:S$initY$0_0$0({1}SC:U),Z,0,0)({7}S:S$posnY$0_0$0({1}SC:U),Z,0,0)({8}S:S$tileY$0_0$0({1}SC:U),Z,0,0)({9}S:S$leapY$0_0$0({2}SI:U),Z,0,0)({11}S:S$drawX$0_0$0({1}SC:U),Z,0,0)({12}S:S$drawY$0_0$0({1}SC:U),Z,0,0)({13}S:S$player_state$0_0$0({1}SC:U),Z,0,0)({14}S:S$player_index$0_0$0({1}SC:U),Z,0,0)({15}S:S$player_frame$0_0$0({1}SC:U),Z,0,0)({16}S:S$player_count$0_0$0({2}SI:U),Z,0,0)]
T:Fcont_screen$tag_struct_scroll_object[({0}S:S$scrollLeftX$0_0$0({1}SC:U),Z,0,0)({1}S:S$scrollRight$0_0$0({1}SC:U),Z,0,0)({2}S:S$scrollColumn$0_0$0({2}SI:U),Z,0,0)({4}S:S$scrollFinish$0_0$0({2}SI:U),Z,0,0)({6}S:S$scrollLeftX0$0_0$0({1}SC:U),Z,0,0)({7}S:S$scrollLeftX1$0_0$0({1}SC:U),Z,0,0)({8}S:S$scrollLeftX2$0_0$0({1}SC:U),Z,0,0)({9}S:S$scrollLeftX3$0_0$0({1}SC:U),Z,0,0)({10}S:S$scrollLeftX4$0_0$0({1}SC:U),Z,0,0)({11}S:S$scrollLeftX5$0_0$0({1}SC:U),Z,0,0)({12}S:S$scroll_half$0_0$0({1}SC:U),Z,0,0)({13}S:S$lineCnt$0_0$0({1}SC:U),Z,0,0)]
T:Fcont_screen$tag_struct_game_object[({0}S:S$game_difficulty$0_0$0({1}SC:U),Z,0,0)({1}S:S$game_world$0_0$0({1}SC:U),Z,0,0)({2}S:S$game_round$0_0$0({1}SC:U),Z,0,0)({3}S:S$game_level$0_0$0({1}SC:U),Z,0,0)({4}S:S$game_point$0_0$0({1}SC:U),Z,0,0)({5}S:S$game_cloud$0_0$0({1}SC:U),Z,0,0)({6}S:S$game_isgod$0_0$0({1}SC:U),Z,0,0)]
T:Fcont_screen$tag_struct_frame_object[({0}S:S$frame_count$0_0$0({2}SI:U),Z,0,0)]
T:Fcont_screen$tag_struct_delay_object[({0}S:S$delay_value$0_0$0({2}SI:U),Z,0,0)({2}S:S$delay_timer$0_0$0({2}SI:U),Z,0,0)]
T:Fcont_screen$tag_struct_level_object[({0}S:S$level_data$0_0$0({2}DG,SC:U),Z,0,0)({2}S:S$level_size$0_0$0({2}SI:U),Z,0,0)({4}S:S$level_bank$0_0$0({1}SC:U),Z,0,0)({5}S:S$column_draw$0_0$0({1}SC:U),Z,0,0)]
T:Fcont_screen$tag_struct_reset_object[({0}S:S$reset_value$0_0$0({1}SC:U),Z,0,0)({1}S:S$reset_timer$0_0$0({1}SC:U),Z,0,0)]
S:Fcont_screen$physics_array$0_0$0({34}DA34d,SC:S),E,0,0
S:G$global_game_object$0_0$0({7}STtag_struct_game_object:S),E,0,0
S:G$level_platforms$0_0$0({0}DA0d,SC:U),E,0,0
S:G$global_level_object$0_0$0({6}STtag_struct_level_object:S),E,0,0
S:G$jump_array_ptr$0_0$0({0}DA0d,DG,SI:S),E,0,0
S:G$global_player_object$0_0$0({18}STtag_struct_player_object:S),E,0,0
S:G$global_scroll_object$0_0$0({14}STtag_struct_scroll_object:S),E,0,0
S:G$global_delay_object$0_0$0({4}STtag_struct_delay_object:S),E,0,0
S:G$global_frame_object$0_0$0({2}STtag_struct_frame_object:S),E,0,0
S:G$global_reset_object$0_0$0({2}STtag_struct_reset_object:S),E,0,0
S:G$engine_collision_manager_init$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_collision_manager_player$0_0$0({2}DF,SC:S),C,0,0
S:G$engine_debug_manager_initgame$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_debug_manager_printout$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_debug_manager_scrollout$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_debug_manager_initcontent$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_font_manager_char$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_font_manager_text$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_font_manager_data$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_font_manager_zero$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_game_manager_init$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_game_manager_set_difficulty$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_input_manager_init$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_input_manager_update$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_input_manager_hold$0_0$0({2}DF,SC:U),C,0,0
S:G$engine_input_manager_move$0_0$0({2}DF,SC:U),C,0,0
S:G$engine_level_manager_init$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_level_manager_draw_point$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_level_manager_draw_column$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_player_manager_init$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_player_manager_initX$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_player_manager_loadX$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_player_manager_loadY$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_player_manager_right$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_player_manager_down$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_player_manager_draw$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_player_manager_pass$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_player_manager_count$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_scroll_manager_init$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_scroll_manager_load$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_scroll_manager_update$0_0$0({2}DF,SC:U),C,0,0
S:G$engine_scroll_manager_section$0_0$0({2}DF,SC:U),C,0,0
S:G$engine_delay_manager_load$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_delay_manager_update$0_0$0({2}DF,SC:U),C,0,0
S:G$engine_delay_manager_draw$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_frame_manager_load$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_frame_manager_update$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_frame_manager_draw$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_reset_manager_load$0_0$0({2}DF,SV:S),C,0,0
S:G$engine_reset_manager_update$0_0$0({2}DF,SC:U),C,0,0
S:G$engine_reset_manager_reset$0_0$0({2}DF,SV:S),C,0,0
