...
void engine_hack_manager_load()
{
	...
	ho->hack_currhp = PEEK( HACKER_START + 1 );			// 0x0050		// Non-zero sets default healthy points.
	ho->hack_goldno = PEEK( HACKER_START + 2 );			// 0x0051		// Non-zero sets default gold available.
	ho->hack_weapon = PEEK( HACKER_START + 3 );			// 0x0052		// Set value to 1 or 2 for stock else 0.
	ho->hack_armors = PEEK( HACKER_START + 4 );			// 0x0053		// Set value to 1 or 2 for armor else 0.
	ho->hack_oneups = PEEK( HACKER_START + 5 );			// 0x0054		// Set value to 1=Life otherwise 0=None.
	ho->hack_musics = PEEK( HACKER_START + 6 );			// 0x0055		// Set 0=Music to play otherwise silent.
	ho->hack_sounds = PEEK( HACKER_START + 7 );			// 0x0056		// Set 0=Sound to play otherwise silent.
